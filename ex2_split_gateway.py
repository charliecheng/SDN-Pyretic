...
class split_gateway(vdef): #check the other split_gateway.py example under ~/pyretic/pyretic/vdef folder first!
    def __init__(self, redo):
        ...
        self.fabric_policy = (
            ...
            (match(vswitch=1000, voutport=4) >> fwd(4)) +
            (match(vswitch=1000, voutport=5) >> fwd(5)) +
            (match(vswitch=1000, voutport=6) >> fwd(6)) +
            
            #If we are destined to a fake switch, 
            #push another header that says which fake switch we are at.
            (match(vswitch=1000, voutport=7) >> push(at="vswitch 1001, vinport 1")) +
            #ATTENTION: complete the other header pushers!!! (see exercise setup)
            #......................
            #switch 1001
            (match(vswitch=1001, voutport=1) >> push(at="vswitch 1000, vinport 7")) +
            (match(vswitch=1001, voutport=2) >> push(at="vswitch 1002, vinport 1")) +

            #switch 1002 other than black-hole side
            ...
            (match(vswitch=1002, voutport=3) >> fwd(8)) +
            #Destined to internet side
            (match(vswitch=1004, voutport=2) >> fwd(7)))
    ...
    def make_vmap(self):
        ...
        mapping.d2u[Location(1000,4)] = [Location(1,4)]
        mapping.d2u[Location(1000,5)] = [Location(1,5)]
        mapping.d2u[Location(1000,6)] = [Location(1,6)]
        mapping.d2u[Location(1000,7)] = [Location(1,None)] #None physical port because virtual port does not map to any physical
        ...
        mapping.d2u[Location(1002,3)] = [Location(1,8)]
        ...
        mapping.d2u[Location(1004,2)] = [Location(1,7)]
        return mapping
    ...
    def set_network(self,network):
        ...
        try:
            ...
            self.derived.topology.add_node(1000, ports={i: Port(i) for i in range(1,7+1)})
            ...
            self.derived.topology.add_link(Location(1001,1),Location(1000,7))  #internal s1001[1] -- s1000[7]
            #......................
